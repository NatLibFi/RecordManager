...
use RecordManager\Database\Database;
...
use RecordManager\Utils\PerformanceCounter;
use RecordManager\Utils\XslTransformation;
...
class RecordManager
{
    ...
    public function __construct($console = false, $verbose = false)
    {
        ...
        $dedupClass = isset($configArray['Site']['dedup_handler'])
            ...
            : '\RecordManager\Deduplication\DedupHandler';
        ...
        if (isset($configArray['Site']['full_title_prefixes'])) {
            MetadataUtils::$fullTitlePrefixes = array_map(
                ['\RecordManager\Utils\MetadataUtils', 'normalize'],
                file(
    ...
    {
        $preview = new \RecordManager\Solr\PreviewCreator(
            $this->db, $this->basePath, $this->log, $this->verbose
        );
